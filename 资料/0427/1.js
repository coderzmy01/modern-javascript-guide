...
/* 
DOM对象：原生JS获取的DOM元素对象，拥有内置的属性和方法
...
JQ选择器：就是为了创建JQ类的一个实例，这样就可以调用jQuery.prototype上提供的属性方法了
  ...
  => 把一个函数当做“普通函数”执行，最后也创造其一个实例
  => 这种模式可以称之为“工厂设计模式”

  要求传递两个实参：selector, context
  + selector是选择器
    + 代表假，例如：""/0/NaN/null/undefined/false，直接返回JQ对象（没有任何的私有属性）
    + 函数：
      $(function(){}) => $(document).ready(function(){}) 
      监听DOM元素是否加载完成「监听DOMContentLoaded」，如果加载完成则触发函数执行
    + DOM对象：把DOM对象转换为JQ对象，这样就可以调用JQ原型上的方法了
      返回一个JQ对象
